defmodule Integration.SlackTest do
  ...
  test "editing a command", %{user: user, client: client} do
    user |> with_permission("operable:st-echo")

    initial = "#{System.os_time()}"
    {:ok, reply} = ChatClient.chat_wait!(client, [room: @ci_room, message: initial, reply_from: @bot,
                                                  edited: "@#{@bot}: operable:st-echo test"])
    assert reply.text == "@#{@user} Executing edited command 'operable:st-echo test'"
    assert reply.location.type == :channel
    assert reply.location.name == @ci_room
    :timer.sleep(500)
    [last_message] = ChatClient.get_messages(client, @bot, 1)
    assert last_message.text == "test"
    assert last_message.location.type == :channel
    assert last_message.location.name == @ci_room
  end
